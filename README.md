...
# Работа с push (проект на GitHub)
1. Создаем локальный репозиторий, сделали работу
2. Синхронизируем удаленный и локальные репозитории
3. Отпраляем (pull) локальный репозиторий на удаленный
4. Были произведены изменения на другом ПК
5. Выкачиваем (push) изменения с удаленного репозитория на локальный

Для тренировки рекомендуется потренироваться на сайте (https://learngitbranching.js.org/?locale=ru_RU)