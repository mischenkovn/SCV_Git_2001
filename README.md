...
## Другие операции
1. **В Git имеется дополнительный механизм сохранения, который называется «stash».** Stash — это временная область для хранения изменений, не готовых к коммиту. Команда stash работает с рабочим каталогом (первым из трех деревьев) и имеет множество вариантов применения.
![stash](stash.png) 
Используется для перехода между ветками без сохранения в коммиты, чтоб не терять данные.
2. **Склеивание коммитов.** Применяется для более чистого вида репозитория, объединяет коммиты в один.
![squash](squash.png) 
Есть два способа применения метода:
* Слияние (merge) с параметром --squash - подтягивает все изменения с указанной ветки, добавляет файлы с изменениями всех коммитов сливаемой ветки, но не создает коммит слияния:
![merge_squash](merge_squash.jpg) 

* 1. Используем команду
```
git rebase -i HEAD~n (где n-нужное число объединяемых коммитов)
```
Например: 
![rebase_squash](rebase_squash.png) 

В результате открывается блокнот, где можно настроить режим объединения коммитов - подробнее см. в документации.

![i_rebase](i_rebase.png) 
Для объединения в нашем случае мы можем поменять в блокноте параметры 2го и 3го коммита на f - fixup. т.е. взять все изменения коммита, внести их в коммит 1 и убрать сообщения из лога, вот так: 

![fixup](fixup.png) 

После выполнения команды при просмотре лога мы увидим теперь только 1 коммит, но при просмотре файла - все изменения, внесенные всеми склеенными коммитами. 

![log_after_rebase](log_after_rebase.png) 

Для фиксации изменения истории в удаленном репозитории команда git push применяется с ключом -- force.

3. **Cherry Pick** - перенос отдельных коммитов, позволяет скопировать изменения указанного коммита и создать новый коммит в текущей ветке.

Команды:

![CherryPick](CherryPick.png) 

4. **Amend - перезапись коммита**

![amend](amend1.png)

Откроется редактор, просто выйти или изменить сообщение коммита.

В истории будет также один коммит, который мы изменили, но с новым хэшем:
