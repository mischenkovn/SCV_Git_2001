...
```
Если для конкретного проекта необходимо представиться по-другому, то следует использовать аргумент *local* (это переопределит глобальные настройки)
```
...
git init
```
Чтобы создать репозиторий в другом каталоге, необходимо указать его в качестве аргумента
```
git init <directory>
```
## Создание репозитория для совместной работы
Следующая команда создает каталог с именем *sharedproject.git* c правами на запись в него
```
...
Вывод истории можно персонализировать при помощи формата
```
...
git branch
```
Данная команда выведет список наших веток, а текущая ветка, в которой мы находимся, будет выделена зеленым цветом и звездочкой.

Для того чтобы удалить ветку необходимо использовать команду:
```
git branch -d <название ветки>
```
Если в этой ветке есть не зафиксированные изменения или коммиты, то Git откажется её удалять. Для того чтобы всё же удалить такую ветку используйте команду:
```
git branch -D <название ветки>
```
...
Если потребовалось создание новой ветки, которая будет начинаться с указанного коммита, выполняем команду:
```
...
Однако, при определённых обстоятельствах, есть смысл использовать команду:
```
