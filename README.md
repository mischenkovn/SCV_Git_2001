...
```
Также можно использовать следующую команду для вывода списка коммитов в виде красивого графа/дерева:
```
git log --graph
```
Еще одна команда, которая выведет все коммиты во всех ветках в виде дерева, где каждый коммит будет в сокращенном виде в одну строку:
```
git log --oneline --all --graph
```
Чтобы посмотреть все коммиты в данной ветке с выводом всех изменений, необходимо ввести команду:
```
git log -p
```
