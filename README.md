...
**!!! Обязательно проверить папку на наличае репозитория командой git status, чтобы избежать создание репозитория в репозитории**
```
git init
```
...
git push -u origin main
```
**!!! Если подружить локальную и удаленную версии не получается, пробуем commit -a -m "Сообщение"**  
