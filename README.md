...
Ветка в Git простой перемещаемый указатель на один из коммитов, обычно последний в цепочке коммитов. По уполчанию, имя основной ветки в Git - `master`.  Создать ветку можно коммандой:
```
...
git clone <адрес удалённого репозитория
```
Сейчас, сделав необходимые правки, сделав commit, загружаем изменения в свой репозиторий на [GitHub](https://github.com/). Для этого используем команды:
```
git add <file.name>
