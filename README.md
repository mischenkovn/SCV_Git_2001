...
```
Для внесения изменений в последний коммит, используется команда
```
git commit --amend -m "comment"
```
Важно, что при изменении создается новый коммит с другим хешем. Поэтому использовать команду нужно до отправки в удаленный репозиторий.
```
