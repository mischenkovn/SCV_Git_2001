...
Краткая таблица необходжимых команд
Команда      | Действие
---------|:-------:
git init     |   Создать репозиторий в открытой папке
git status  |   Проверка статуса репозитория
git remote add «origin» «ссылка»     |   Связать с удалённым репозиторием. Origin это название большинства удалённых источников
git remote -v | Показать список подключеных соединений 
git add [«Имя файла»]| Добавление файла в будущий комит
git add . | Добавление всех файлов без перечисления в коммит
git rm --cached «Имя файла» | Удаление файла из комита
git diff | После сохрания файла, до записи комита, показывает внесённые изменения касательно последнего коммита
git commit [-m “комментарий”] [-am “комментарий”] [--amend] | Создание коммита Ключ m для комментария. Ключ am для добавления файла в коммит и записи комментария одновременно (не работает на директории). Перезаписать последний коммит если он ещё не отправлен в удалённый репозиторий
git log [--oneline] [--graph] [-p] |Посмотреть историю коммитов. Команда oneline для более компактной истории. Команда graph отображает лог с ветками. Ключ p чтобы показать с выводом изминений
git reset «хэш номер» [--hard] | Сброс до определённого момента. Сброс на последний коммит
git reflog | Посмотреть историю всех операций 
git revert «хэш номер» | Создание нового коммита отменяющее действия совершённые в указаном
git restore «хэш номер» | Cброс состояние файла на указанное
git branch [название новой ветки] [название ветки с которой нужно сделать копию] [-d «название ветки»] [-f «название ветки» «хэш»] | Просмотр веток. Создание новой ветки. Создание новой ветки на  подобии уже указаной. Ключ d позволяет удалить ненужную ветку. Ключ D позволяет удалить пустую ветку. Ключ f – принудительный сброс ветки к коммиту
git chechout [-b] «название ветки» [название ветки с которой нужно сделать копию] | Переключиться на другую ветку. Кдюч b для одновременного создания новой ветки. Создание новой ветки наподобии уже указаной
git chechout [master]| Master команда перекинет в самую новую версию
git push [-u origin «название ветки»] |Выгрузить на гитхаб. Выгрузить с созданием ветки если такой нет
git push [--delete origin «название ветки»] | Выгрузить и удалить ветки с удаленного репозитория
git push [--force] | Принудительная перезапись истории после коррекции
git push [--tags] | Загрущзить имеено тэги
git push [--delete origin «название ветки»] | Удалить тэг с гитхаб
git pull | Загрузить с гитхаба обновления ветки на которой находишься
git fetch | Загрузить обновления всех веток с удаленного репозитория
git merge «название ветки» [--squash] |С указаной ветки, на ветку на которой находишься влить все изменения. Подтягивает обновления с указаной ветки, но не создаёт коммит обьединение
git rebace «название ветки» [-i HEAD ~ «чиссло комитов»] | Перенос начала ветки на последний комит указаной ветки. Склеить комиты в один
git tag [«1.0.0»] [--list] [-d «1.0.0»] |Узнать текущую версию. Записать последний коммит как версию. Посмотреть список всех тэгов. Удалить указаный тэг
git stash [list] [push «название файла»] [pop «название стеша»] | Выгрузить все изменения во временное хранилище, а не в комит. Посмотреть перечень стешей. Добавить файл к стешу. Вернуть из стеша на ветку на которой находишься
git cherry-pick «хэш номер» [«имя ветки»][.. «имя ветки»] |Перенос конкретного комита на ветку. Перенос последнего комита с ветки. Перенос всех комитов с ветки
