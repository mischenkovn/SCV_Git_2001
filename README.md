...
Но ошибки случаются и необходимо, например, отменить слияние веток (git merge).
Есть два решения:
- удаление самого коммита слияния командами `unmerge, merge undo, merge delete`;
- отмена изменений слияния командой `revert` (команду лучше использовать для отмены общих публичных изменений) и `resert` (команду лучше использовать для отмены частных локальных изменений).

Первый случай поможет, если вы еще не сделали новых коммитов, после слияния веток, и не отправили изменения в ваш центральный репозиторий.
...
git reset --hard HEAD^
!!! В этом случае отменяются изменения, которые были сделаны в файле
