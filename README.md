...
Благодаря данной команде мы можем не только добавлять новые файлы в репозиторий, но и выбирать части этого самого файла, которые будут добавлены в следующей версии (коммит), вот как выглядит написание комманды:
```
git add -p
```
Комманда представит вам фрагмент изменений и предложит вам действия для дальнейшей работы с новой версией. Введите `y`, чтобы добавить фрагмент; `n`, чтобы игнорировать фрагмент; `s`, чтобы разбить его на более мелкие фрагменты; `e`, чтобы вручную отредактировать фрагмент; `q`, чтобы завершить работу с командой.
...
```
Комманда `git commit -m "comment"` является быстрой командой, которая создает коммит с указаным комментарием. По умолчанию команда git commit без параметра `-m` открывает локально настроенный текстовый редактор `Vim` с предложением ввести комментарий к коммиту. В это же время при передаче параметра `-m` текстовый редактор не открывается, а используется подставленный комментарий.

У многих людей, только начинающих работу с Visual Studio Code, возникают проблемы с выходом из текстового редактора `Vim`. Давайте разберем по пунктам, что делать в случае если нам необходимо выйти из `Vim`:
1. Перевести раскладку клавиатуры на **_Английский язык_**.
2. Нажать кнопку **_ESC_**.
3. Набрать в терминале команду **_:x_**.
4. Поздравляю вы вышли из `Vim`.

Чтобы создать `commit` только отслеживаемых файлов (тех, которые были в какой-то момент работы добавлены в историю с помощью команды `git add`) используется следующая команда:
```
git commit -a
```
Комбинация параметров `-a` и `-m` позволяет создать `commit` всех изменений отслеживаемого файла и сразу создает комментарий, без блужданий в текстовом редакторе `Vim`:
```
git commit -a -m "comment"
```
Также в `Git` есть возможность редактирования последнего коммита, для этого используется комманда:
```
git commit --amend
```
Вместо создания нового коммита изменения, которые мы произвели в файле, будут сохранены в последнем коммите. Также после ввода данной программы, есть возможность изменить указанный ранее комментарий к коммиту.
