...
Так же по веткам можно перемещаться, указывая конкретный коммит, или где он находится относительно текущего.
```
git branch -f master 1234567  # переместить ветку master на указанный коммит
